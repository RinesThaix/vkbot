...
public abstract class AbstractStrategy {
    ...
    protected final int[][] bot = new int[10][10],
            players = new int[10][10],
            bot_pw = new int[10][10],
            players_pw = new int[10][10];

    protected final static int NOT_CHECKED = 0, //неизвестно, что здесь
    ...
    private int shipCellsLeft_bot = 20,
    ...
    public static String getMatrix(int[][] matrix) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (int j = 0; j < 10; ++j) {
                int v = matrix[j][i];
                sb.append(v == PLACED_SHIP || v == KILLED_SHIP || matrix[j][i] == WOUNDED_SHIP ? SYMBOL_SHIP
                                : v == NOT_CHECKED ? SYMBOL_NOT_CHECKED
                                : v == SURELY_EMTPY ? SYMBOL_SURELY_EMPTY : SYMBOL_OTHER);
            }
            sb.append("\n");
        }
        return sb.toString();
